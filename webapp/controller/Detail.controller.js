		...
		return BaseController.extend("sap.trainings.controller.Detail", {
			...
			oModel.create('/Participants', oEntry, null, function() {
				...
				onRegisterSubmit: function() {
					...
					var partID = Math.floor((1 + Math.random()) * 0x10000);
                    ...
                    if (fname === "" || lname === "" || email === "" || title === "" ) {
                        sap.m.MessageToast.show('Please fill up Registration form!');
                        return;
                    }
                    
                    var oEntry = {};
					oEntry.TrainingId = oObject.TrainingId;
					oEntry.ParticipantID = partID.toString();
					oEntry.FirstName = fname;
					oEntry.LastName = lname;
					oEntry.Email = email;
					oEntry.Title = title;
					oEntry.Phone = " ";
					oEntry.RegisterFlag = "X";
					oEntry.cust_1 = " ";
					oEntry.cust_2 = " ";
				...
				validateInput: function() {
					...
					if (fname === ""){
						sap.ui.getCore().byId("__input7").setValueState(sap.ui.core.ValueState.Error); }
					else {
						sap.ui.getCore().byId("__input7").setValueState(sap.ui.core.ValueState.Success);
				    }
				    
				    if (lname === ""){
						sap.ui.getCore().byId("__input8").setValueState(sap.ui.core.ValueState.Error); }
					else {
						sap.ui.getCore().byId("__input8").setValueState(sap.ui.core.ValueState.Success);
				    }
				    
				    if (email === ""){
						sap.ui.getCore().byId("__input9").setValueState(sap.ui.core.ValueState.Error); }
					else {
						sap.ui.getCore().byId("__input9").setValueState(sap.ui.core.ValueState.Success);
				    }
				    
				    if (title === ""){
						sap.ui.getCore().byId("__input10").setValueState(sap.ui.core.ValueState.Error); }
					else {
						sap.ui.getCore().byId("__input10").setValueState(sap.ui.core.ValueState.Success);
				    }
				}
