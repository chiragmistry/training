...
sap.ui.define([
	...
	return BaseController.extend("sap.trainings.controller.Master", {
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
			    var aSearchLocal = [new Filter("ShortDesc", FilterOperator.Contains, sQuery), 
					                new Filter("TrainerName", FilterOperator.Contains, sQuery)
				                                     ]; 
				this._oListFilterState.aSearch = new Filter(aSearchLocal, false);
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("sap.trainings.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("RemSeats", FilterOperator.LE, 10));
						break;
					case "Filter2":
						aFilters.push(new Filter("RemSeats", FilterOperator.GT, 20));
						break;
					default:
						break;
				}
		...
		_onMasterMatched: function() {
			this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
				function(mParams) {
					...
					var sObjectId = mParams.firstListitem.getBindingContext().getProperty("TrainingId");
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext().getProperty("TrainingId")
			}, bReplace);
		},
