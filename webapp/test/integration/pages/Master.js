				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("TrainingId") === oObject.TrainingId;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].TrainingId;
										} else {
											sCurrentId = aItemsNotInTheList[0].TrainingId;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Trainings", {
											TrainingId : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("TrainingId"),
							title: oBindingContext.getProperty("ShortDesc")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Trainings (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Trainings (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("RemSeats");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("ShortDesc");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Trainings to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Trainings (" + iExpectedLength + ")"}),
									success : function () {
